...
namespace DataHubWeb2.Model {
    ...
    */
    ...
    public static class DataProvider {
        ...
        public static List<ReportRow> GetIssues() {
            if(HttpContext.Current.Session["Issues"] == null)
                HttpContext.Current.Session["Issues"] = GenerateIssues();
            return HttpContext.Current.Session["Issues"] as List<ReportRow>;
        }
        static void UpdateIssues(List<ReportRow> list) {
            HttpContext.Current.Session["Issues"] = list;
        }

        private static readonly object lockObject = new object();
        public static void AddNewIssue(ReportRow issue) {
            lock(lockObject) {
                /*
                List<ReportRow> issues = GetIssues();
        ...
        public static void UpdateIssue(ReportRow issue) {
            /*
            List<ReportRow> issues = GetIssues();

            ReportRow existingIssue = issues.Find(i => i.Id == issue.Id);
        ...
        static List<ReportRow> GenerateIssues()
        {            
            List<ReportRow> issueList = new List<ReportRow>();

            foreach (var reportBase in AuditData.GetReports(AuthHelper.Employee.WindowsUser))
            {
                issueList.Add(new ReportRow()
                {
    ...
    public static class SchedulerLabelsHelper {
        ...
        static SchedulerLabelsHelper() 
        {
            ...
            foreach (var task in AuditData.GetProjectEmployeeAssocs().Where(p => p.EmployeeId == AuthHelper.Employee.DBID))
            {
    ...
    public class AppointmentDataSourceHelper {
        ...
        private static List<SchedulerAppointment> GenerateAppointments() 
        {
            ...
            foreach (var task in AuditData.GetProjectEmployeeAllocs().Where(p => p.EmployeeId == AuthHelper.Employee.DBID))
            {
                var project = AuditData.GetProject(task.ProjectId);
                string resources = "";
                resources = AuthHelper.Employee.DBID;
    ...
    public static class ResourceDataSourceHelper {
        ...
        static ResourceDataSourceHelper() 
        {
            ...
            var employee = AuthHelper.Employee;
