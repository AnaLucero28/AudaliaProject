    ...
    */
    ...
    public static class DataProvider
    {
        public static List<Report> GetReports()
        {
            if (HttpContext.Current.Session["Reports"] == null)
                HttpContext.Current.Session["Reports"] = GenerateReports();
            return HttpContext.Current.Session["Reports"] as List<Report>;
        }
        ...
        */
        ...
        static void UpdateReports(List<Report> list)
        {
            HttpContext.Current.Session["Reports"] = list;
        }
        ...
        */
        ...
        public static void UpdateReport(Report Report)
        {
            List<Report> Reports = GetReports();
            /*
            Report existingReport = Reports.Find(i => i.DBID == Report.DBID);
            if (existingReport != null)
                existingReport.Assign(Report);
            */
        ...
        {
            List<Report> Reports = GetReports();
            Reports.RemoveAll(i => ids.Contains(i.Id));
            UpdateReports(Reports);
        }
        ...
        */
        ...
        static List<Report> GenerateReports()
        {
            List<Report> reports = new List<Report>();

            foreach (var reportBase in AuditData.GetReports())
            {
                reports.Add(new Report() {
                    Id = long.Parse(reportBase.DBID), 
    ...
    public static class SchedulerLabelsHelper
    {
        ...
        static SchedulerLabelsHelper()
        {
            ...
            foreach (var task in AuditData.GetProjectEmployeeAssocs().Where(p => p.EmployeeId == "12"))
            {
                var project = AuditData.GetProject(task.ProjectId);
                var color = project.Color.StringToColor();
                if (color == Color.Transparent)
                {
                    var r = Math.Max(0, 128 * rnd.Next(0, 3) - 1);
                    var g = Math.Max(0, 128 * rnd.Next(0, 3) - 1);
                    var b = Math.Max(0, 128 * rnd.Next(0, 3) - 1);
                    color = System.Drawing.Color.FromArgb(r, g, b);
                }

                items.Add(new SchedulerLabel { Id = int.Parse(project.DBID), Name = project.Name, Color = color });
            }
        }
    ...
    public class AppointmentDataSourceHelper
    {
        ...
        public static List<SchedulerAppointment> CreateAppointments()
        {
            ...
            System.Security.Principal.WindowsIdentity user = HttpContext.Current.Request.LogonUserIdentity;

            foreach (var employee in AuditData.GetEmployees().Where(p => p.WindowsUser.ToLower() == user.Name.ToLower()))
                foreach (var task in AuditData.GetProjectEmployeeAllocs().Where(p => p.EmployeeId == employee.DBID))
                {
                    var project = AuditData.GetProject(task.ProjectId);
                    string resources = "";
                    ...
                    /*
                    var taskStartDate = project.StartDate.TruncStartDate();
                    var taskFinishDate = project.FinishDate.TruncFinishDate().AddDays(-0.5);
                    */
                    var taskStartDate = task.AllocDate.AddHours(8);
                    var taskFinishDate = task.AllocDate.AddHours(18);


                    appointments.Add(new SchedulerAppointment()
                    {
                        Id = ++id, //task.DBID,
                        //ResourceId = int.Parse(employee.DBID), //resources,
                        AllDay = false, //true,
                        //Modified = false,
                        ...
                        ResourceId = int.Parse(project.DBID),
                        LabelId = int.Parse(project.DBID)
                    });


                }
            /*
            foreach (var task in AuditData.GetProjectEmployeeAssocs().Where(p => p.EmployeeId == employee.DBID)) 
            {
                var project = AuditData.GetProject(task.ProjectId);
                string resources = "";
                resources = employee.DBID;
    ...
    public static class ResourceDataSourceHelper
    {
        ...
        static ResourceDataSourceHelper()
        {
            ...
            System.Security.Principal.WindowsIdentity user = HttpContext.Current.Request.LogonUserIdentity;            
            var employee = AuditData.GetEmployees().FirstOrDefault(p => p.WindowsUser.ToLower() == user.Name.ToLower());
            ...
            if (employee != null)
            {
                ...
                {
                    var project = AuditData.GetProject(task.ProjectId);
                    items.Add(new SchedulerResource { Id = int.Parse(project.DBID), Name = project.Name });
                }
            }
        }
